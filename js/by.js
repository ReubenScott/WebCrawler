  var BDUrlBefore="[www.vxiaotou.com]";
  var BDUrlLast="";
  var install="/bdyy.html";  
  var buffer="/loading.html";  
  var pause="/zt.html";  
...
function addFlash(url,w,h){
  ...
  if(url.indexOf('bdhd://')==0){
	...
	if(browser == "Microsoft Internet Explorer"){
	  ...
	  //document.write("<param name='LastWebPage' value='"+prehref+"'>");
	  //document.write("<param name='NextWebPage' value='"+nexthref+"'>");
	  ...
	  if (!install){
		...
		document.write('<object id="BaiduPlayer" name="BaiduPlayer" type="application/player-activex" width="'+w+'" height="'+h+'" progid="Xbdyy.PlayCtrl.1" param_URL="'+url+'" param_OnPlay="onPlay" param_OnPause="onPause" param_OnFirstBufferingStart="onFirstBufferingStart" param_OnFirstBufferingEnd="onFirstBufferingEnd" param_OnPlayBufferingStart="onPlayBufferingStart" param_OnPlayBufferingEnd="onPlayBufferingEnd" param_OnComplete="onComplete" param_Autoplay="1" param_ShowStartClient="1"></object>');
	  }else{
		  BdhdInstall(w,h);
	  }
	}else{
